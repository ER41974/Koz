...
/// Detect a wake word at the end of the transcribed text.
/// Returns (cleaned_text, matching KozMode) if a wake word is found.
fn detect_wake_word(text: &str, settings: &AppSettings) -> Option<(String, KozMode)> {
    if !settings.wake_word_enabled || settings.wake_word_prefix.is_empty() {
        return None;
    }

    let prefix = &settings.wake_word_prefix;
    // Build a case-insensitive regex: \s*{prefix}\s+(\S+)\s*$
    let pattern = format!(r"(?i)\s*{}\s+(\S+)\s*$", regex::escape(prefix));
    let re = match Regex::new(&pattern) {
        Ok(re) => re,
        Err(e) => {
            warn!("Invalid wake word regex: {}", e);
            return None;
        }
    };
    ...
    if let Some(caps) = re.captures(text) {
        ...
        // Find a matching mode by wake_suffix
        if let Some(mode) = settings
            .koz_modes
            .iter()
            .find(|m| m.enabled && m.wake_suffix.eq_ignore_ascii_case(suffix))
        {
            let full_match = caps.get(0).unwrap();
            let cleaned = text[..full_match.start()].trim_end().to_string();
            debug!(
                "Wake word detected: '{}' -> mode '{}' (suffix '{}')",
                full_match.as_str(),
                mode.name,
                suffix
            );
            return Some((cleaned, mode.clone()));
        }
    }
    ...
    fn stop(&self, app: &AppHandle, binding_id: &str, _shortcut_str: &str) {
        ...
        tauri::async_runtime::spawn(async move {
            ...
            if let Some(samples) = rm.stop_recording(&binding_id) {
                ...
                debug!(
                    "Recording stopped and samples retrieved in {:?}, sample count: {}, duration: {:.1}s",
                        ...
                        if !transcription.is_empty() {
                            ...
                            let (should_post_process, override_prompt_id) =
                                if let Some((cleaned, mode)) = detect_wake_word(&final_text, &settings) {
                                    ...
                                    debug!(
                                        "Koz mode '{}' activated (post_process={})",
                                        mode.name, needs_pp
                                    );
                                    (needs_pp, pid)
                                } else {
                                    // No wake word: use original binding behavior
                                    (post_process, None)
                                };

                            // Apply LLM post-processing if needed
                            ...
                            if let Some(processed_text) = processed {
                                ...
                                let used_prompt_id = override_prompt_id
                                    .or_else(|| settings.post_process_selected_prompt_id.clone());
                            ...
                            ah.run_on_main_thread(move || {
                                ...
                                // Show green "Done" icon briefly, then revert to Idle
                                change_tray_icon(&ah_clone, TrayIconState::Done);
                                let ah_timer = ah_clone.clone();
                                tauri::async_runtime::spawn(async move {
                                    tokio::time::sleep(std::time::Duration::from_secs(2)).await;
                                    change_tray_icon(&ah_timer, TrayIconState::Idle);
                                });
                            })
                            .unwrap_or_else(|e| {
